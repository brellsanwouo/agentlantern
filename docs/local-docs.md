...
This site is built with [VitePress](https://vitepress.dev/). These commands are only for people editing AgentLantern's own product documentation, not for using AgentLantern on an agent project.
...
[VitePress](https://vitepress.dev/) builds static files that work on GitHub Pages:
